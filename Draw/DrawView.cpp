...
BEGIN_MESSAGE_MAP(CDrawView, CView)
	...
	ON_COMMAND(ID_CLEAR, &CDrawView::OnClear)
END_MESSAGE_MAP()
...
void CDrawView::OnChoice()
{
	...
	//设置位置
    m_CpenDlg->SetWindowPos(&CWnd::wndTop, rect.right - 320, rect.top + 150, 0, 0, SWP_NOSIZE);
	// 显示非模态对话框   
...
void CDrawView::OnMouseMove(UINT nFlags, CPoint point)
{
	...
	if (shape == 4)
	{
		if ((nFlags == MK_LBUTTON) && is_leftButton_down)
		{
			...
			r = sqrt((float)((chosen_position.x - instant_position.x)*(chosen_position.x - instant_position.x)
			...
			pDC->MoveTo(chosen_position);
			pDC->LineTo(point);
			r = sqrt((float)((chosen_position.x - point.x)*(chosen_position.x - point.x)
